...
const gameName = "leasing_kalkulator_challenge";
const htmlFile = `_ghtml_${gameName}.html`;
const jsonFile = `_gg01_${gameName}.json`;
const enginePath = "../../../../../../../../game_pages/SimulationGame/index.html";
...
if (fs.existsSync(htmlFile)) {
    ...
    if (content.includes("window.location.replace") && content.includes("SimulationGame")) {
        console.log("✅ HTML Redirect wrapper exists and points to Engine.");
    } else {
        console.error("❌ HTML file exists but does not seem to act as a redirect to SimulationGame.");
        process.exit(1);
    }
} else {
...
if (fs.existsSync(resolvedEnginePath)) {
    console.log("✅ SimulationGame Engine found.");
} else {
    console.error(`❌ SimulationGame Engine NOT found at ${resolvedEnginePath}`);
    process.exit(1);
}
