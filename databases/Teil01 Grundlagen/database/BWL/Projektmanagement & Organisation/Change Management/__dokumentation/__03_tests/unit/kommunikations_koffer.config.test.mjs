...
const configPath = path.join(gameDir, '_data/_gg01_kommunikations_koffer.json');
...
describe('Kommunikations-Koffer Config', () => {
  it('contains rounds with exactly one correct option each', async () => {
    ...
    for (const [index, round] of cfg.rounds.entries()) {
      assert.ok(Array.isArray(round.options), `round ${index + 1}: missing options array`);
      assert.ok(round.options.length >= 3, `round ${index + 1}: expected at least 3 options`);
