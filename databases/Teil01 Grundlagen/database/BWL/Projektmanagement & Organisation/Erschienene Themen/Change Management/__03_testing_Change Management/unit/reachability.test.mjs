...
describe('Local Reachability Smoke Test', () => {
  it('has a sibling __02_doing folder with at least one game asset', async () => {
    ...
    const gameFiles = files.filter((name) => /^(?:game_|_game_|_ghtml_|_gjs_).+\.(html|js|json)$/i.test(name));
    assert.ok(gameFiles.length > 0, 'expected at least one playable _g*/game_* html/js/json file');
  });
});
