(function () {
  ...
  async function init() {
    try {
      const resp = await fetch('_data/_gg01_konflikt_loeser.json');
  ...
  function render() {
    ...
    el.root.innerHTML = `
      <h2>Konfliktfall (${escapeHtml(round.conflict_type)})</h2>
      <div class="conflict">${escapeHtml(round.prompt)}</div>
  ...
  window.render_game_to_text = function renderGameToText() {
    ...
    return JSON.stringify({
      mode: state.done ? 'result' : 'conflict_strategy',
      coordinate_system: 'origin top-left, x right, y down',
      round_index: state.idx,
      total_rounds: state.cfg?.rounds.length || 0,
      conflict_type: round?.conflict_type || null,
      score: state.score,
      streak: state.streak,
