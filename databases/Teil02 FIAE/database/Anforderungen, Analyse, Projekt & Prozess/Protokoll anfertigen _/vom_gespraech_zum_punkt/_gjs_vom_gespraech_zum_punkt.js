(function () {
  ...
  async function init() {
    try {
      const response = await fetch('_data/_gg01_vom_gespraech_zum_punkt.json');
  ...
  function render() {
    ...
    if (state.done) {
      el.root.innerHTML = `
        ...
        <ul class="result-list">
          <li>Gute Aktionspunkte enthalten Verantwortliche.</li>
          <li>Gute Aktionspunkte enthalten eine Frist.</li>
          <li>Formulierungen muessen knapp und eindeutig sein.</li>
        </ul>
        <div class="actions">
    ...
    el.root.innerHTML = `
      <h2>${escapeHtml(round.id.toUpperCase())}: Beste Kurzform waehlen</h2>
      <div class="statement">${escapeHtml(round.statement)}</div>
      <div class="answers">
  ...
  function answer(selected) {
    ...
    feedback.innerHTML = state.lastCorrect
      ...
      : `Nicht optimal. Richtige Kurzform: <strong>${escapeHtml(correctOption.text)}</strong>. ${escapeHtml(round.reason)}`;
  ...
  window.render_game_to_text = function renderGameToText() {
    ...
    return JSON.stringify({
      mode: state.done ? 'result' : 'action_point_compression',
      ...
      statement: round?.statement || null
    });
  };
