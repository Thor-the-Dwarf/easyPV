(function () {
  ...
  async function init() {
    const resp = await fetch('_data/_gg01_kuendigungsgrund_klassifikator.json');
    if (!resp.ok) {
      el.card.textContent = 'Konfigurationsdatei konnte nicht geladen werden.';
      return;
    }
  ...
  function render() {
    ...
    const options = round.options.map(function (opt) {
      return '<button class="option-btn" type="button" data-id="' + opt.id + '" ' + (state.answered ? 'disabled' : '') + '>' + opt.text + '</button>';
    }).join('');
    ...
    el.card.innerHTML =
      ...
      '<div class="actions">' +
      '  <button id="next-btn" class="action-btn" type="button" hidden>' + (state.index === state.cfg.rounds.length - 1 ? 'Auswertung' : 'Naechste Runde') + '</button>' +
      '</div>';

    Array.from(el.card.querySelectorAll('[data-id]')).forEach(function (btn) {
      btn.addEventListener('click', function () {
        answer(btn.getAttribute('data-id'));
      });
    });
  ...
  function answer(optionId) {
    ...
    const round = state.cfg.rounds[state.index];
    const option = round.options.find(function (entry) { return entry.id === optionId; });
    if (!option) return;

    state.answered = true;
    state.selectedCorrect = !!option.correct;

    if (state.selectedCorrect) {
      state.score += Number(state.cfg.scoring.correct) || 0;
    } else {
      state.score += Number(state.cfg.scoring.wrong) || 0;
    }

    const feedback = document.getElementById('feedback');
    feedback.hidden = false;
    feedback.className = 'feedback ' + (state.selectedCorrect ? 'ok' : 'bad');
    feedback.textContent = option.feedback || (state.selectedCorrect ? 'Korrekt.' : 'Nicht korrekt.');
