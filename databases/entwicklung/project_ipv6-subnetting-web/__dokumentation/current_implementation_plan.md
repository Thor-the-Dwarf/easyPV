# Implementierungsplan – Session 26-02-20 (WP-01 bis WP-03)
...
- Drawer-Metadaten erweitert (`assets/layout.js`): eigene Accordion-Einträge für `prefix-calculator`, `network-range`, `prefix-split-enumerator`.
- Tool-Factory erweitert (`assets/tools.js`): Mounting über `createPrefixCalculator(...)`, `createNetworkRange(...)`, `createPrefixSplitEnumerator(...)`.
- Neue Widgets:
  - `assets/lib/prefix-calculator.js`
  - `assets/lib/network-range.js`
  - `assets/lib/prefix-split-enumerator.js`
- Core-Lib erweitert (`assets/lib/ipv6.js`): `addressCountForPrefix()`, `subnetCountBetween()`, `prefixRange()`, `enumerateSubprefixes()`.
...
- Lernpfad aktiviert (`assets/data/lessons.json`):
  ...
  - `subnetting/prefix-basics` zeigt `prefix-split-enumerator`, `network-range`, `prefix-slicer`
