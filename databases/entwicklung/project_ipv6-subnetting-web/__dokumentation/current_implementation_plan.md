# Implementierungsplan – Session 26-02-20 (WP-01 bis WP-05)
...
- Tool-Factory erweitert (`assets/tools.js`): Mounting über `createPrefixCalculator(...)`, `createNetworkRange(...)`, `createPrefixSplitEnumerator(...)`, `createNextPreviousNetwork(...)`, `createContainmentOverlapCheck(...)`.
- Neue Widgets:
...
- Core-Lib erweitert (`assets/lib/ipv6.js`): `addressCountForPrefix()`, `subnetCountBetween()`, `prefixRange()`, `enumerateSubprefixes()`, `nextPreviousNetwork()`, `containmentOverlapCheck()`.
- Tests erweitert (`assets/lib/ipv6.test.js`).
- Lernpfad aktiviert (`assets/data/lessons.json`):
  - `basics/hex-prefix` zeigt `prefix-calculator`, `network-range`, `prefix-split-enumerator`, `next-previous-network`, `containment-overlap-check`, `prefix-visualizer`
  - `subnetting/prefix-basics` zeigt `prefix-split-enumerator`, `next-previous-network`, `containment-overlap-check`, `network-range`, `prefix-slicer`
