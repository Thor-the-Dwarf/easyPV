(function () {
  ...
  async function init() {
    try {
      const response = await fetch('_data/_gg01_betreffzeilen_bingo.json');
  ...
  function evaluate(optionId) {
    ...
    if (selected === 'timeout') {
      ...
      state.feedback = 'Zeit abgelaufen. ' + (correctOption ? 'Beste Option: "' + correctOption.text + '".' : '');
      render();
      return;
    }
    ...
    if (isCorrect) {
      ...
      state.feedback = option && option.feedback ? option.feedback : 'Richtig ausgewaehlt.';
      ...
      state.feedbackState = 'bad';
      state.feedback = option && option.feedback ? option.feedback : 'Diese Option ist zu unpraezise.';
      if (correctOption) {
        state.feedback += ' Beste Option: "' + correctOption.text + '".';
      }
    }
  ...
  function render() {
    ...
    if (state.done) {
      ...
      el.root.innerHTML = [
        ...
        '<p>Qualitaetsquote: <strong>' + escapeHtml(String(computeRatePercent())) + '%</strong></p>',
    ...
    html += '<header class="mail-head"><div class="mail-title">Mail-Auszug</div></header>';
    html += '<p class="mail-body">' + escapeHtml(round.bodyPreview || '') + '</p>';
    html += '</article>';
    ...
    if (state.feedback) {
      ...
      html += state.roundIndex >= state.cfg.rounds.length - 1 ? 'Auswertung' : 'Naechste Mail';
      html += '</button></div>';
    }
  ...
  function asStatePayload() {
    const round = currentRound();
    return {
      mode: state.done ? 'result' : 'subject_line_quiz',
      ...
      current_task: round
        ? {
            id: round.id,
            body_preview: round.bodyPreview,
