(function () {
  ...
  async function init() {
    try {
      const response = await fetch('_data/_gg01abc1_verteiler_puzzle.json');
  ...
  function render() {
    ...
    if (state.done) {
      el.root.innerHTML = `
        ...
        <ul class="result-list">
          <li>Verteiler nach Betroffenheit und Entscheidungskompetenz waehlen.</li>
          <li>Wichtige Rollen direkt aufnehmen, irrelevante auslassen.</li>
          <li>Ziel: Informationsfluss statt Informationsrauschen.</li>
        </ul>
        <div class="actions">
    ...
    el.root.innerHTML = `
      <h2>${escapeHtml(round.id.toUpperCase())}: Wer soll das Protokoll erhalten?</h2>
  ...
  function answer(selected) {
    ...
    feedback.innerHTML = state.lastCorrect
      ...
      : `Nicht korrekt. Passender Verteiler: <strong>${escapeHtml(correctOption.text)}</strong>. ${escapeHtml(round.reason)}`;
  ...
  window.render_game_to_text = function renderGameToText() {
    ...
    return JSON.stringify({
      mode: state.done ? 'result' : 'distribution_list_selection',
