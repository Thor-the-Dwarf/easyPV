(function () {
  ...
  async function init() {
    try {
      const response = await fetch('_data/_gg01_prozess_schnittstellen.json');
  ...
  function render() {
    ...
    if (state.done) {
      el.root.innerHTML = `
        ...
        <ul class="result-list">
          <li>Input beschreibt, womit ein Schritt startet.</li>
          <li>Output beschreibt, was der Schritt liefert.</li>
          <li>Klare Schnittstellen verhindern Medienbrueche.</li>
        </ul>
        <div class="actions">
    ...
    el.root.innerHTML = `
      <h2>${escapeHtml(round.id.toUpperCase())}: Input/Output bestimmen</h2>
      <div class="step"><strong>Prozessschritt:</strong> ${escapeHtml(round.process_step)}</div>
      <div class="options">
  ...
  function answer(selected) {
    ...
    const correct = round.options.find((option) => option.id === round.correct_option);
    ...
    feedback.innerHTML = state.lastCorrect
      ...
      : `Nicht korrekt. Richtige Zuordnung: <strong>${escapeHtml(correct.text)}</strong>. ${escapeHtml(round.reason)}`;
  ...
  window.render_game_to_text = function renderGameToText() {
    ...
    return JSON.stringify({
      mode: state.done ? 'result' : 'process_interface_match',
      ...
      process_step: round?.process_step || null
    });
  };
