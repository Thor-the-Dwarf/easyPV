(function () {
  ...
  async function init() {
    const resp = await fetch('_data/_gg01_kuendigungsfrist_timer.json');
  ...
  function render() {
    ...
    Array.from(el.card.querySelectorAll('[data-id]')).forEach((btn) => btn.addEventListener('click', () => answer(btn.getAttribute('data-id'))));
  ...
  function answer(optionId) {
    ...
    if (option.correct && Number.isFinite(Number(state.cfg.scoring.reason_bonus)) && option.reason_ok === true) {
      state.score += Number(state.cfg.scoring.reason_bonus) || 0;
    }
  ...
  window.render_game_to_text = function renderGameToText() {
    return JSON.stringify({
      mode: 'mietvertrag_kuendigungsfrist_timer',
      measurable: true,
