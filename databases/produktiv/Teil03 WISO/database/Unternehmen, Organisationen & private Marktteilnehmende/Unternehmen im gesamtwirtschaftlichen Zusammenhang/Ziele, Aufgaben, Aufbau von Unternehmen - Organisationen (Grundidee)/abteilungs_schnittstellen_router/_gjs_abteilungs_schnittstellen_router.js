(function () {
  ...
  async function init() {
    const resp = await fetch('_data/_gg01_abteilungs_schnittstellen_router.json');
    if (!resp.ok) {
  ...
  function render() {
    ...
    const options = round.options.map(function (opt) {
      return '<button class="option-btn" type="button" data-id="' + opt.id + '" ' + (state.answered ? 'disabled' : '') + '>' + escapeHtml(opt.text) + '</button>';
    }).join('');
  ...
  window.render_game_to_text = function renderGameToText() {
    return JSON.stringify({
      mode: 'abteilungs_schnittstellen_router',
      measurable: true,
